var GameCamera = (function () {
	...
	GameCamera.prototype.update = function() {
		...
		// var offset = 2
		var v4 = tube.spline.getPointAt(Math.abs(curLoopTime-offset-offset*fovdelta)%1);
		var v4d = tube.spline.getTangentAt(Math.abs(curLoopTime-offset)%1);
		...
		var v4n = tube.spline.getPointAt((curLoopTime+offset*7)%1);
		var v4nd = tube.spline.getTangentAt((curLoopTime+offset*7)%1);
		...
		this.camera.position = this.calculatedPosition.clone().add(cross.applyAxisAngle(this.calculatedDirection,this.angle*TO_RADIANS).multiplyScalar(tube.radius-this.height));
